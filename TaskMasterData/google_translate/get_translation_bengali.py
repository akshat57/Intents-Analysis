...
file1 = open('taskmaster_bengali_intent_text.txt','a')
hindi_translation = []
for key in utterances:
    if int(key) >= 0:
        print(key)
        translated_text = translate_text(utterances[key], 'bn')
...
with open('taskmaster_bengali_intent_text_full.txt', 'w') as filehandle:
